...
from ...utils.github import github_download_file
from ...utils.mixins import SaveImageWithUniqueNameMixin
from ...utils.path import posix_to_pypath
from .face_analysis import FixedFaceAnalysis
...
class InstantID(FaceSwapperModel, SaveImageWithUniqueNameMixin):
    ...
    def _check_or_download_models_files(self, force_download: bool = False):
        ...
        for fp in tqdm(self.FILEPATHS_FOR_DOWNLOADING, desc=bar_desc, leave=False):
            if not fp.is_file() or force_download:
                hf_hub_download(repo_id="InstantX/InstantID", filename=fp.as_posix(), local_dir=self.models_weight_path)
    ...
    def prepare_model(self, force_download: bool = False):
        ...
        self.controlnet = ControlNetModel.from_pretrained(
            os.path.join(self.models_weight_path, self.CONTROLNET_DIR), torch_dtype=torch.float16
        )
        ...
        self._pipeline = StableDiffusionXLInstantIDPipeline.from_pretrained(
            pretrained_model_name_or_path=self.REALIVIZ_NAME, controlnet=self.controlnet, torch_dtype=torch.float16
        )
    ...
    def swap_face(
        self, source_image_path, face_emb: np.ndarray, prompt: str, gender: FaceSwapperModel.Gender | str
    ) -> str:
        global draw_kps
        ...
        face_kps = draw_kps(empty_img, pose)  # noqa
